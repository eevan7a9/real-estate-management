...
export class ChangePasswordComponent implements OnInit {
  ...
  constructor(
    ...
    this.changePassForm = this.fb.group({
      ...
      passwordConfirm: [
        '',
        [...passwordValidators, CustomValidators.confirmPasswordValidator]
      ]
    });
  }
  ...
  public async submit(): Promise<void> {
    if (this.changePassForm.invalid) {
      ...
      if (this.changePassForm.controls.passwordConfirm?.errors?.PasswordNoMatch)
    ...
    try {
      const res = await firstValueFrom(
        this.user.changePassword(passwordNew, passwordCurrent)
      );
